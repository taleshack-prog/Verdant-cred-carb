...
export class CarbonMintEntity {
  ...
  @Column({ name: 'tx_hash', nullable: true, length: 66 }) txHash: string | null;
  @Column({ name: 'raw_data_hash', length: 66 }) rawDataHash: string;
  @Column({ name: 'validation_status', default: 'PENDING', length: 20 }) validationStatus: string;
